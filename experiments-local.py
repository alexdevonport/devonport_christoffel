...
def comparison_experiment_local(sampler, nx, epsilon_target, delta, experiment_name='exp'):
    ...
    cfun_nys = KernelCfun(kfun=kfun_se, threshold=threshold_se, noiselevel=noiselevel_se, delta=delta, nx=2, n_nys=2000)
...
logging.basicConfig(
        handlers=[
            logging.FileHandler("{:s}-experiments-local.log".format(timestr)),
