...
#postcov = np.linalg.inv(np.linalg.inv(kmat) + sig0**(-1)*imat)

print('Computing KL divergence')
kl = kl_centered_normal(postcov, priorcov)
print('KL divergence: {:.5g}'.format(kl))

print('Computing KL divergence (alternate formula)')
evec = np.linalg.eigvalsh(kmat)
ld = np.sum(np.log(1+sig0**(-1)*evec))
tr = np.sum((1 + sig0**(-1)*evec)**(-1)) - nsamps
print(ld,tr)
klnew = 0.5*(ld + tr)
print('new KL divergence: {:.5g}'.format(klnew))
